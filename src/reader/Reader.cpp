...
namespace OpenLogReplicator {
    ...
    uint64_t Reader::checkBlockHeader(uint8_t* buffer, typeBlk blockNumber, bool showHint) {
        ...
        if ((blockSize == 512 && buffer[1] != 0x22) ||
            ...
            ctx->OLR_ERROR(40001, "file: " + fileName + " - block: " + std::to_string(blockNumber) + " - invalid block size: " +
        ...
        if (sequence == 0 || status == STATUS_UPDATE) {
            ...
            if (group == 0) {
                if (sequence != sequenceHeader) {
                    ctx->OLR_WARN(60024, "file: " + fileName + " - invalid header sequence, found: " + std::to_string(sequenceHeader) +
        ...
        if (blockNumberHeader != blockNumber) {
            ctx->OLR_ERROR(40002, "file: " + fileName + " - invalid header block number: " + std::to_string(blockNumberHeader) +
        ...
        if (!ctx->disableChecksSet(Ctx::DISABLE_CHECKS_BLOCK_SUM)) {
            ...
            if (chSum != chSumCalculated) {
                if (showHint) {
                    ctx->OLR_WARN(60025, "file: " + fileName + " - block: " + std::to_string(blockNumber) +
    ...
    uint64_t Reader::reloadHeaderRead() {
        ...
        if (actualRead < 512) {
            ctx->OLR_ERROR(40003, "read file: " + fileName + " - " + strerror(errno));
        ...
        // Check file header
        if (headerBuffer[0] != 0) {
            ctx->OLR_ERROR(40003, "file: " + fileName + " - invalid header[0]: " + std::to_string(static_cast<uint64_t>(headerBuffer[0])));
            return REDO_ERROR_BAD_DATA;
        }
        ...
        if (headerBuffer[28] == 0x7A && headerBuffer[29] == 0x7B && headerBuffer[30] == 0x7C && headerBuffer[31] == 0x7D) {
            ...
            ctx->OLR_ERROR(40004, "file: " + fileName + " - invalid header[28-31]: " + std::to_string(static_cast<uint64_t>(headerBuffer[28])) +
        ...
        if (!blockSizeOK) {
            ctx->OLR_ERROR(40005, "file: " + fileName + " - invalid block size: " + std::to_string(blockSize) + ", header[1]: " +
        ...
        if (actualRead > 0 && ctx->redoCopyPath.length() > 0) {
            ...
            if (fileCopyDescriptor == -1) {
                ...
                ctx->OLR_INFO(0, "writing redo log copy to: " + fileNameWrite);
            ...
            if (bytesWritten != actualRead) {
                ctx->OLR_ERROR(10007, "file: " + fileNameWrite + " - " + std::to_string(bytesWritten) + " bytes written instead of " +
    ...
    uint64_t Reader::reloadHeader() {
        ...
        if ((compatVsn >= 0x0B200000 && compatVsn <= 0x0B200400)        // 11.2.0.0 - 11.2.0.4
            ...
            ctx->OLR_ERROR(40006, "file: " + fileName + " - invalid database version: " + std::to_string(compatVsn));
            return REDO_ERROR_BAD_DATA;
        }
        ...
        if (numBlocksHeader != Ctx::ZERO_BLK && fileSize > static_cast<uint64_t>(numBlocksHeader) * blockSize && group == 0) {
            ...
            ctx->OLR_INFO(0, "updating redo log size to: " + std::to_string(fileSize) + " for: " + fileName);
        }
        ...
        if (ctx->version == 0) {
            ...
            ctx->OLR_INFO(0, "found redo log version: " + ctx->versionStr + ", activation: " + std::to_string(activation) + ", resetlogs: " +
        ...
        if (version != ctx->version) {
            ctx->OLR_ERROR(40007, "file: " + fileName + " - invalid database version: " + std::to_string(compatVsn) + ", expected: " +
        ...
        if (unlikely(ctx->trace & Ctx::TRACE_DISK))
            ctx->OLR_TRACE(Ctx::TRACE_DISK, "block: 1 check: " + std::to_string(retReload));
        ...
        if (firstScn == Ctx::ZERO_SCN || status == STATUS_UPDATE) {
            ...
            if (firstScnHeader != firstScn) {
                ctx->OLR_ERROR(40008, "file: " + fileName + " - invalid first scn value: " + std::to_string(firstScnHeader) + ", expected: " +
        ...
        if (nextScn == Ctx::ZERO_SCN && nextScnHeader != Ctx::ZERO_SCN) {
            if (unlikely(ctx->trace & Ctx::TRACE_DISK))
                ctx->OLR_TRACE(Ctx::TRACE_DISK, "updating next scn to: " + std::to_string(nextScnHeader));
            nextScn = nextScnHeader;
        } else if (nextScn != Ctx::ZERO_SCN && nextScnHeader != Ctx::ZERO_SCN && nextScn != nextScnHeader) {
            ctx->OLR_ERROR(40009, "file: " + fileName + " - invalid next scn value: " + std::to_string(nextScnHeader) + ", expected: " +
    ...
    bool Reader::read1() {
        ...
        if (toRead == 0) {
            ctx->OLR_ERROR(40010, "file: " + fileName + " - zero to read, start: " + std::to_string(bufferStart) + ", end: " +
        ...
        if (unlikely(ctx->trace & Ctx::TRACE_DISK))
            ctx->OLR_TRACE(Ctx::TRACE_DISK, "reading#1 " + fileName + " at (" + std::to_string(bufferStart) + "/" +
        ...
        for (uint64_t numBlock = 0; numBlock < maxNumBlock; ++numBlock) {
            ...
            if (unlikely(ctx->trace & Ctx::TRACE_DISK))
                ctx->OLR_TRACE(Ctx::TRACE_DISK, "block: " + std::to_string(bufferScanBlock + numBlock) + " check: " +
        ...
        if (goodBlocks == 0 && group == 0) {
            if (nextScnHeader != Ctx::ZERO_SCN) {
                ...
                ctx->OLR_WARN(60023, "file: " + fileName + " - position: " + std::to_string(bufferScan) + " - unexpected end of file");
                ret = REDO_STOPPED;
            }
            return false;
        }
    ...
    bool Reader::read2() {
        ...
        if (goodBlocks > 0) {
            ...
            if (toRead == 0) {
                ctx->OLR_ERROR(40011, "zero to read (start: " + std::to_string(bufferStart) + ", end: " + std::to_string(bufferEnd) +
            ...
            if (unlikely(ctx->trace & Ctx::TRACE_DISK))
                ctx->OLR_TRACE(Ctx::TRACE_DISK, "reading#2 " + fileName + " at (" + std::to_string(bufferStart) + "/" +
                                               std::to_string(bufferEnd) + "/" + std::to_string(bufferScan) + ") bytes: " + std::to_string(toRead));
            ...
            if (actualRead > 0 && fileCopyDescriptor != -1) {
                int64_t bytesWritten = pwrite(fileCopyDescriptor, redoBufferList[redoBufferNum] + redoBufferPos, actualRead,
                                              static_cast<int64_t>(bufferEnd));
                if (bytesWritten != actualRead) {
                    ctx->OLR_ERROR(10007, "file: " + fileNameWrite + " - " + std::to_string(bytesWritten) +
            ...
            for (uint64_t numBlock = 0; numBlock < maxNumBlock; ++numBlock) {
                ...
                if (unlikely(ctx->trace & Ctx::TRACE_DISK))
                    ctx->OLR_TRACE(Ctx::TRACE_DISK, "block: " + std::to_string(bufferEndBlock + numBlock) + " check: " +
                                                   std::to_string(currentRet));
        ...
        while (!ctx->softShutdown) {
            {
                ...
                if (status == STATUS_SLEEPING && !ctx->softShutdown) {
                    if (unlikely(ctx->trace & Ctx::TRACE_SLEEP))
                        ctx->OLR_TRACE(Ctx::TRACE_SLEEP, "Reader:mainLoop:sleep");
                    ...
                    // Buffer full
                    if (unlikely(ctx->trace & Ctx::TRACE_SLEEP))
                        ctx->OLR_TRACE(Ctx::TRACE_SLEEP, "Reader:mainLoop:buffer");
            ...
            if (status == STATUS_CHECK) {
                if (unlikely(ctx->trace & Ctx::TRACE_FILE))
                    ctx->OLR_TRACE(Ctx::TRACE_FILE, "trying to open: " + fileName);
                redoClose();
                ...
                if (unlikely(ctx->trace & Ctx::TRACE_DISK))
                    ctx->OLR_TRACE(Ctx::TRACE_DISK, "reading " + fileName + " at (" + std::to_string(bufferStart) + "/" +
                ...
                while (!ctx->softShutdown && status == STATUS_READ) {
                    ...
                    if (bufferEnd == fileSize) {
                        if (nextScnHeader != Ctx::ZERO_SCN) {
                            ...
                            ctx->OLR_WARN(60023, "file: " + fileName + " - position: " + std::to_string(bufferScan) +
                    ...
                    if (bufferStart + ctx->bufferSizeMax == bufferEnd) {
                        ...
                        if (!ctx->softShutdown && bufferStart + ctx->bufferSizeMax == bufferEnd) {
                            if (unlikely(ctx->trace & Ctx::TRACE_SLEEP))
                                ctx->OLR_TRACE(Ctx::TRACE_SLEEP, "Reader:mainLoop:bufferFull");
                            condBufferFull.wait(lck);
                            continue;
                        }
                    }
    ...
    void Reader::run() {
        if (unlikely(ctx->trace & Ctx::TRACE_THREADS)) {
            ...
            ctx->OLR_TRACE(Ctx::TRACE_THREADS, "reader (" + ss.str() + ") start");
        }
        ...
        if (unlikely(ctx->trace & Ctx::TRACE_THREADS)) {
            std::ostringstream ss;
            ss << std::this_thread::get_id();
            ctx->OLR_TRACE(Ctx::TRACE_THREADS, "reader (" + ss.str() + ") stop");
        }
    }
    ...
    bool Reader::checkRedoLog() {
        ...
        while (status == STATUS_CHECK) {
            if (ctx->softShutdown)
                break;
            if (unlikely(ctx->trace & Ctx::TRACE_SLEEP))
                ctx->OLR_TRACE(Ctx::TRACE_SLEEP, "Reader:checkRedoLog");
        ...
        for (;;) {
            ...
            while (status == STATUS_UPDATE) {
                if (ctx->softShutdown)
                    break;
                if (unlikely(ctx->trace & Ctx::TRACE_SLEEP))
                    ctx->OLR_TRACE(Ctx::TRACE_SLEEP, "Reader:updateRedoLog");
                condParserSleeping.wait(lck);
            }
    ...
    bool Reader::checkFinished(uint64_t confirmedBufferStart) {
        ...
        if (confirmedBufferStart == bufferEnd) {
            if (ret == REDO_STOPPED || ret == REDO_OVERWRITTEN || ret == REDO_FINISHED || status == STATUS_SLEEPING)
                return true;
            if (unlikely(ctx->trace & Ctx::TRACE_SLEEP))
                ctx->OLR_TRACE(Ctx::TRACE_SLEEP, "Reader:checkFinished");
            condParserSleeping.wait(lck);
        }
        return false;
    }
