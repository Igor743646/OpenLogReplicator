...
namespace OpenLogReplicator {
    ...
    void LobCtx::checkOrphanedLobs(const Ctx* ctx, const typeLobId& lobId, typeXid xid, uint64_t offset) {
        ...
        for (auto orphanedLobsIt = orphanedLobs->upper_bound(lobKey);
            ...
            if (unlikely(ctx->trace & Ctx::TRACE_LOB))
                ctx->OLR_TRACE(Ctx::TRACE_LOB, "id: " + lobId.lower() + " page: " + std::to_string(orphanedLobsIt->first.page));
    ...
    void LobCtx::addLob(const Ctx* ctx, const typeLobId& lobId, typeDba page, uint64_t pageOffset, Lob data, typeXid xid, uint64_t offset) {
        ...
        if (dataMapIt != lobData->dataMap.end()) {
            if (unlikely(ctx->trace & Ctx::TRACE_LOB))
                ctx->OLR_TRACE(Ctx::TRACE_LOB, "id: " + lobId.lower() + " page: " + std::to_string(page) + " OVERWRITE");
        }
