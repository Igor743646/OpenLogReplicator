...
namespace OpenLogReplicator {
    ...
    uint64_t CharacterSet::badChar(const Ctx* ctx, typeXid xid, uint64_t byte1) const {
        ctx->OLR_WARN(60008, "can't decode character: (" + std::to_string(byte1) + ") using character set " + name + ", xid: " +
                            xid.toString());
    ...
    uint64_t CharacterSet::badChar(const Ctx* ctx, typeXid xid, uint64_t byte1, uint64_t byte2) const {
        ctx->OLR_WARN(60008, "can't decode character: (" + std::to_string(byte1) + ", " + std::to_string(byte2) +
